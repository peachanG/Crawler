...
parser.add_argument('--overwrite', '-w', action='store_true',
                    help='if readed csv have text_colums_name in config,' +
                         'delete columns and overwrite')
...
def main():
    ...
    for i, file_path in enumerate(file_path_list):
        ...
        # dfの中にtext_colums_nameのcolumnがもうすでにあったら処理をしない。
        if text_colums_name in list(df.columns.values):
            if args.overwrite:
                df = df.drop(columns=[text_colums_name])
            else:
                continue
